...
sys.path.append('.')
# Cityscapes imports
from evaluator.cityscapesscripts.helpers.csHelpers import *
from evaluator.cityscapesscripts.evaluation.instances2dict import instances2dict, appendInstanceDict
...
def getGtInstances(groundTruthList, args):
    ...
    orig_len = len(gtInstances)
    for gt in groundTruthList:
        if gt not in gtInstances:
            appendInstanceDict(gt, gtInstances)
    if orig_len != len(gtInstances):
        writeDict2JSON(gtInstances, args.gtInstancesFile)
...
def evaluate(prediction, gt, prediction_dir, stat_dir=None):
    global args
    if stat_dir:
        args.exportFile = stat_dir
    args.predictionPath = os.path.abspath(prediction_dir)
    res_dict = evaluateImgLists(prediction, gt, args)
    ap = res_dict['averages']['allAp']
    return ap
